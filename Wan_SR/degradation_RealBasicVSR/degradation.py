...
from Wan_SR.degradation_RealBasicVSR.blur_kernels import random_mixed_kernels
...
class RandomResize:
    ...
    def _random_resize(self, imgs):
        ...
        if target_size is None:
            ...
            resize_step = 0
            # if target_size is provided, set target_size为target_size到img的size围内的随机数
            h_target, w_target = target_size
            # Ensure we get a valid range
            h_min, h_max = min(h_target, h), max(h_target*2, h)
            w_min, w_max = min(w_target, w), max(w_target*2, w)
            # Generate random size within the range
            h_out = np.random.uniform(h_min, h_max)
            w_out = np.random.uniform(w_min, w_max)
            target_size = (int(h_out), int(w_out))
